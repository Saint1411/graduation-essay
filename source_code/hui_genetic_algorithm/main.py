...
class AlgoHUIMGA:
    def __init__(self):
        ...
        self.twu_pattern = []   # The items which has twu value more than minUtil
        self.writer = None
        self.population = []
        self.sub_population = []
        self.hui_sets = []
            ...
            for line in file:
                ...
                if line and not line.startswith(("#", "%", "@")):
                    ...
                    for item in items:
                        ...
                        twu0 = self.map_item_to_twu0.get(item, 0) + transaction_utility
    ...
    def roulette_percent(self):
        percentage = []
        total_sum = sum(self.map_item_to_twu.values())
        temp_sum = 0
    ...
    #         self.hui_sets.append(HUI(itemset, temp_chro_node.fitness))
    #     else:
    #         for i in range(len(self.hui_sets)):
    #             if itemset == self.hui_sets[i].itemset:
    #                 break
    #         else:
    #             self.hui_sets.append(HUI(itemset, temp_chro_node.fitness))
    ...
    def insert(self, temp_chro_node):
        ...
        if not any(hui.itemset == itemset for hui in self.hui_sets):
            self.hui_sets.append(HUI(itemset, temp_chro_node.fitness))
    ...
    def calculate_r_fitness(self):
        total_sum = sum(node.fitness for node in self.population)
        temp_sum = 0
    ...
    def print_result(self, output_path=False):
