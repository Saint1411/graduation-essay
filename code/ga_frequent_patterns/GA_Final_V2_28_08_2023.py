...
def exists(transaction, candidate,item_candidate):
    i = 0; n = len(candidate); m = len(transaction)
    while i<n and candidate[i]==0: i+=1 #tìm nhiễm sắc thể đầu tiên = 1
    if i==n: return False #nếu không tìm thấy
    j=0 #duyệt từ đầu transaction
    while i<n: #duyệt trên candidate
        ##print(i)
        while j<m and transaction[j] != item_candidate[i]: j+=1; #trong khi chưa có xuất hiện ở transaction
        ##print(j)
        if j==m: return False #nếu không tìm thấy
        i+=1; j+=1 #qua phần tử kế
        while i<n and candidate[i]==0: i+=1
    return True
...
def find_frequent_itemsets(population, dataset, support_count,item_candidate):
    fitness_scores = [fitness_function(dataset, individual,item_candidate) for individual in population]
    frequent_itemsets=[]
...
def KhoiTaoBitArr(population_size, item_candidate):
    population=[]; i=0; maxNumber = len(item_candidate) 
    while(i<population_size):
        individual = []
        for _ in range(maxNumber):
            individual.append(random.randint(0, 1))
        if sum(individual)==0: continue
...
#Thuật toán chính
def GV_Slide(transactions,population_size, support_count,batch):
    all_frequent_itemsets = []
    for i in range(0,len(transactions), batch):
        # window_data = transactions[i:i+window_size]
        frequent_itemsets = GA(generations,population_size,support_count)
...
min_s = 0.25 
support_count = min_s # window_size * min_support * confidence
print("****DOING****")
...
s = time.time()
# all_frequent_itemsets = GV_Slide(data, population_size, support_count,batch)
all_frequent_itemsets = GA(generations, population_size, dataset, support_count)
e = time.time()
print("*********")
print('Min sup: ', min_s)
